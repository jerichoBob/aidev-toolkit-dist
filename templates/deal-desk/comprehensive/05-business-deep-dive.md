# Business Deep Dive

**Analysis Date:** {{DATE}}<br/>
**Scope:** R5 (Financial), R9 (Organizational), R10 (Market/Business) risks

## Financial Analysis

### Contract Economics

| Metric | Value | Risk |
|--------|-------|------|
| Total Contract Value | {{TCV}} | {{RISK}} |
| Payment Terms | {{TERMS}} | {{RISK}} |
| Margin Estimate | {{MARGIN}} | {{RISK}} |
| Cash Flow Impact | {{CASHFLOW}} | {{RISK}} |

### Cost Drivers

1. **{{DRIVER_1}}**: {{DESCRIPTION}}
2. **{{DRIVER_2}}**: {{DESCRIPTION}}
3. **{{DRIVER_3}}**: {{DESCRIPTION}}

### Financial Scenarios

| Scenario | Assumptions | Outcome |
|----------|-------------|---------|
| Best Case | {{ASSUMPTIONS}} | {{OUTCOME}} |
| Expected | {{ASSUMPTIONS}} | {{OUTCOME}} |
| Worst Case | {{ASSUMPTIONS}} | {{OUTCOME}} |

## R5: Financial Risks - Detailed Analysis

### R5.1: {{RISK_TITLE}}

**Risk Statement:** {{DETAILED_DESCRIPTION}}

**Financial Impact:**

- Potential cost: {{COST}}
- Probability: {{PROBABILITY}}
- Expected value: {{EV}}

**Recommended Approach:**

{{RECOMMENDED_APPROACH}}

---

## R9: Organizational Risks - Detailed Analysis

### R9.1: {{RISK_TITLE}}

**Risk Statement:** {{DETAILED_DESCRIPTION}}

**Stakeholder Impact:**

| Stakeholder Group | Impact | Change Required | Readiness |
|-------------------|--------|-----------------|-----------|
| {{GROUP}} | {{IMPACT}} | {{CHANGE}} | {{READINESS}} |

**Change Management Considerations:**

- {{CONSIDERATION_1}}
- {{CONSIDERATION_2}}

**Recommended Approach:**

{{RECOMMENDED_APPROACH}}

---

## R10: Market/Business Risks - Detailed Analysis

### R10.1: {{RISK_TITLE}}

**Risk Statement:** {{DETAILED_DESCRIPTION}}

**Strategic Context:**

{{STRATEGIC_CONTEXT}}

**Competitive Landscape:**

- {{COMPETITOR_1}}: {{POSITION}}
- {{COMPETITOR_2}}: {{POSITION}}

**Recommended Approach:**

{{RECOMMENDED_APPROACH}}

---

## Business Recommendations

### Strategic Considerations

1. {{RECOMMENDATION_1}}
2. {{RECOMMENDATION_2}}

### Commercial Leverage Points

1. {{LEVERAGE_1}}
2. {{LEVERAGE_2}}
